...
exports.getTransactions = async (req, res) => {
  try {
    ...
    if (cachedResults) {
      ...
      // caching results
      await redisClient.set(id + "-transactions",JSON.stringify(transactions), {
        EX: 3600,
      });
...
exports.processTransaction = async (req,res) => {
  ...
  try {
    ...
    if(!receiver){
      throw new Error("No user found with phone number: "+req.body.phoneNumber)
    }
    ...
    let {msg,cashbackAmount} = cashback_helper(req.body.amount,newTransactionRecord.snd_BankBalance)
